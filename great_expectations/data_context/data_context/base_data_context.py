...
class BaseDataContext(EphemeralDataContext, ConfigPeer):
    ...
    def validate_config(cls, project_config: Union[DataContextConfig, Mapping]) -> bool:
        """
        Class method to validate config against schema. Run as one of the first steps of BaseDataContext __init__()
        Args:
            project_config (DataContextConfig, Mapping): config to check

        Returns:
            Should return True if the configuration is valid. Error if not.
        """
